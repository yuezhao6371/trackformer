...
def load_dataloader(config, device, mode='all'):
    ...
    if mode == 'train' or mode == 'all':
        ...
        loaders['train'] = DataLoader(train_dataset, batch_size=batch_size, shuffle=shuffle, num_workers=num_workers, collate_fn=collate_fn)
        loaders['val'] = DataLoader(val_dataset, batch_size=batch_size, num_workers=num_workers, collate_fn=collate_fn)
    ...
    if mode == 'eval' or mode == 'all':
        ...
        loaders['test'] = DataLoader(test_dataset, batch_size=batch_size, num_workers=num_workers, collate_fn=collate_fn)
        loaders['test_helper'] = DataLoader(test_helper_dataset, batch_size=batch_size, num_workers=num_workers, collate_fn=collate_fn)
